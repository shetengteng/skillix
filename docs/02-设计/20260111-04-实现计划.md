...
- [ ] sx_triage 工具（智能分流）
- [ ] 自动检测重复模式
- [ ] 自动建议功能（集成在 sx_triage 中）

> **注意**：原设计中的 `sx_suggest`、`sx_feedback`、`sx_analyze` 已整合到 `sx_triage` 工具和配置系统中，不再作为独立工具实现。
...
> | 工具名 | 说明 | 包含操作 |
> |--------|------|----------|
> | `sx_skill` | 技能管理 | list, read, create, update, delete |
> | `sx_config` | 配置管理 | get, set, init, sources |
> | `sx_help` | 帮助信息 | 显示使用说明和示例 |
> | `sx_market` | 市场操作 | search, install, uninstall |
> | `sx_triage` | 智能分流 | 分析任务，推荐技能 |
